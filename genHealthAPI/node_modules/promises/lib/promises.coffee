...
# The promise of a an action to be resolved with methods to respond to that action after it is finished. The action
# may happen synchronously or asynchronously.

class Promise
	
	# Allows responding to an action once it is resolved or rejected. Allows responding to progress updates as well.
...
class Deferred
	...
	then: (resolvedHandler, rejectedHandler, progressHandler, canceledHandler) =>
		...
		@_addHandler(resolvedHandler, rejectedHandler, canceledHandler).nextDeferred = nextDeferred
		@_notify(@handlers.pop()) if @finished()
	...
	_finish: (status, results) ->
		return if @status isnt 'pending'  # throw new Error('Deferred has already been ' + this.status);
		clearTimeout @_timeout
		@status = status
		@results = results
	...
	_notify: (handler) ->
		...
		if not method
			deferred[@status](results...) if deferred
			return
...
# shortcuts to create synchronous rejected and resolved promises, e.g. for cached results
